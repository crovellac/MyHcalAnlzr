...
int main(int argc, char *argv[])
{
  ...
  TNtuple* qiedigi10 = (TNtuple*)f->Get("MyHcalAnlzr/qiedigi10");
  int ntot = qiedigi10->GetEntries();
  std::cout << "Reading in input file, total " << ntot << " Entries." << std::endl;
  float RunNum, LumiNum, EvtNum, ieta, iphi, depth, sumADC, type, shunt;
  qiedigi10->SetBranchAddress("RunNum", &RunNum);
  qiedigi10->SetBranchAddress("LumiNum", &LumiNum);
  qiedigi10->SetBranchAddress("EvtNum", &EvtNum);
  qiedigi10->SetBranchAddress("ieta", &ieta);
  qiedigi10->SetBranchAddress("iphi", &iphi);
  qiedigi10->SetBranchAddress("depth", &depth);
  qiedigi10->SetBranchAddress("sumADC", &sumADC);
  qiedigi10->SetBranchAddress("type", &type);
  qiedigi10->SetBranchAddress("shunt", &shunt);
    ...
    for(int t=0; t<2; t++){ // 0=Small SiPM, 1=Large SiPM
      ...
      for(int i=0; i<58; i++){ // Number of ieta bins -29..29, excluding 0
        ...
        for(int j=0; j<72; j++){ // Number of iphi bins 1..72
          ...
          for(int k=0; k<2; k++){ // Number of depths 1..2
